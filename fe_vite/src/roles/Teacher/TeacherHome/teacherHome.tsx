...
const MAIN_TABS = [
  {
    ...
    label: "Quản lý lớp",
    description: "Tạo lớp, thêm học sinh, chia sẻ bài giảng",
  ...
  },
  {
    icon: PiExam,
    label: "Quản lý đề",
    description: "Tạo, chỉnh sửa và giao đề thi cho lớp",
    link: "/teacher/exam/management",
    iconBg: "bg-blue-50",
    iconColor: "text-blue-700",
  },
  {
    ...
    description:
      "Xem điểm số, phân tích bài làm và đưa ra nhận xét cho học sinh",
    link: "/teacher/class/management",
    iconBg: "bg-blue-50",
    iconColor: "text-blue-700",
  },
  {
    ...
    description: "Nhận xét tự động và tư vấn giảng dạy",
    link: "/teacher/class/management",
    iconBg: "bg-blue-50",
    iconColor: "text-blue-700",
  },
...
const STEPS = [
  "Tạo lớp học, tìm kiếm và thêm học sinh vào lớp",
  "Soạn đề thi, cấu hình đề và giao cho lớp",
  "Xem thống kê kết quả, đưa ra nhận xét và nhận tư vấn từ hệ thống",
];

const TeacherHome: React.FC = () => {
  const [tenGiaoVien, setTenGiaoVien] = useState<string>("");

  useEffect(() => {
    const fetchName = async () => {
      const res = await UserAPI.getInfo();
      if (res?.data?.ten) {
        setTenGiaoVien(res.data.ten);
      } else {
        const userString = localStorage.getItem("user");
        const user = userString ? JSON.parse(userString) : null;
        setTenGiaoVien(user?.tenDangNhap ?? "Giáo viên");
      }
    };
    fetchName();
  }, []);
    ...
    <div className="mx-auto w-4/5 max-w-4xl py-10 text-gray-800">
      ...
      <div className="mb-8 grid grid-cols-2 gap-4 md:grid-cols-4">
        {MAIN_TABS.map((tab, index) => (
          ...
          >
            <div
              className={`mb-3 flex h-10 w-10 items-center justify-center rounded-lg ${tab.iconBg}`}
            >
