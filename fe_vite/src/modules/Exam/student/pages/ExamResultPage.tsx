...
const ExamResultPage = () => {
  ...
  // Thêm 2 state để quản lý việc đóng/mở ô nhận xét
  ...
  return (
    <div className="min-h-screen bg-slate-50 px-4 py-10">
      <div className="mx-auto w-full max-w-2xl rounded-2xl bg-white p-8 shadow-sm border border-slate-100">
        <h1 className="text-2xl font-bold text-slate-900 text-center mb-6">Kết quả bài thi</h1>
        ...
        <div className="grid grid-cols-2 gap-4 rounded-xl border border-slate-200 bg-slate-50 p-5 text-slate-700">
          <div className="space-y-2">
            <p><span className="font-semibold text-slate-500 text-sm uppercase">Học sinh</span><br/><span className="text-lg font-medium">{studentName || "Không có dữ liệu"}</span></p>
            <p><span className="font-semibold text-slate-500 text-sm uppercase">Số câu đúng</span><br/><span className="text-lg">{result.soCauDung}/{result.tongSoCau}</span></p>
          </div>
          <div className="space-y-2">
            <p><span className="font-semibold text-slate-500 text-sm uppercase">Điểm số</span><br/><span className="text-3xl font-bold text-blue-600">{result.diemSo}</span></p>
            <p><span className="font-semibold text-slate-500 text-sm uppercase">Thời gian làm</span><br/><span className="text-md">{formatDuration(result.thoiGianLamGiay)}</span></p>
          </div>
        </div>
        ...
        <div className="mt-6 space-y-3">
          ...
          <div className="border border-slate-200 rounded-lg overflow-hidden bg-white shadow-sm">
            ...
            >
              <span className="flex items-center gap-2">🤖 <span>Phân tích từ Hệ thống</span></span>
              <span className="text-xs font-semibold text-slate-500 bg-slate-200 px-2 py-1 rounded">{isOpenSystemFB ? 'Thu gọn ▲' : 'Xem chi tiết ▼'}</span>
            </button>
            
            {isOpenSystemFB && (
              <div className="p-5 border-t border-slate-200 text-slate-700 text-sm whitespace-pre-wrap leading-relaxed bg-white">
                {result.nhanXetHeThong || "Hệ thống chưa có đánh giá cho bài làm này."}
              </div>
            )}
          </div>
          ...
          <div className="border border-blue-200 rounded-lg overflow-hidden bg-white shadow-sm">
            ...
            >
              <span className="flex items-center gap-2">👩‍🏫 <span>Lời khuyên từ Giáo viên</span></span>
              <span className="text-xs font-semibold text-blue-600 bg-blue-100 px-2 py-1 rounded">{isOpenTeacherFB ? 'Thu gọn ▲' : 'Xem chi tiết ▼'}</span>
            </button>
            
            {isOpenTeacherFB && (
              <div className="p-5 border-t border-blue-100 text-sm leading-relaxed bg-white">
                {result.nhanXetGiaoVien && result.nhanXetGiaoVien.trim() !== "" ? (
                  <p className="text-slate-800 whitespace-pre-wrap font-medium">{result.nhanXetGiaoVien}</p>
                ) : (
                  <p className="text-slate-400 italic flex items-center justify-center py-2">Giáo viên chưa để lại nhận xét cho bài làm này.</p>
                )}
              </div>
            )}
          </div>
