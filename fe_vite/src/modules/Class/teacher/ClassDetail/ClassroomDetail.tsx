...
import StudentManagement from "./StudentManagement"; // ⚠️ Đảm bảo đường dẫn import này đúng với vị trí file của ông
...
const ClassroomDetail = () => {
  ...
  const [activeTab, setActiveTab] = useState<"students" | "lectures" | "exams">("students");
      ...
      <div className="flex-1 p-8 overflow-y-auto">
        ...
        {activeTab === "lectures" && (
          <div className="bg-white rounded-xl shadow-md border border-slate-200 p-10 text-center">
            <h2 className="text-2xl font-bold text-slate-800 mb-2">Giao diện Bài Giảng</h2>
            <p className="text-slate-500">Khu vực này sẽ code danh sách và thêm bài giảng vào đây!</p>
          </div>
        )}
        ...
        {activeTab === "exams" && (
          <div className="bg-white rounded-xl shadow-md border border-slate-200 p-10 text-center">
            <h2 className="text-2xl font-bold text-slate-800 mb-2">Giao diện Đề Thi</h2>
            <p className="text-slate-500">Chỗ này để tạo câu hỏi trắc nghiệm nè!</p>
          </div>
        )}
      </div>
