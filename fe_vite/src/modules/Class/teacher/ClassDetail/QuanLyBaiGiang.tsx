...
// Gọi cái form Tạo Bài Giảng của ông vào đây
import FormBaiGiang from './CreateLecture'; 

// 🚀 ĐÃ SỬA LỖI ĐƯỜNG DẪN: Phải lùi ra 4 bước mới tới được thư mục services/apis
// (Nếu thư mục của ông tên là 'apis' thì đổi chữ 'services' thành 'apis' nhé)
  ...
  return (
    <div className="p-6 bg-white rounded-lg min-h-screen">
      <div className="flex justify-between items-center mb-6 border-b pb-4">
        <h2 className="text-2xl font-bold text-gray-800">Danh sách bài giảng</h2>
        <button 
      ...
      <div className="bg-gray-50 rounded-lg shadow-inner p-4 border border-gray-200">
         {danhSachBaiGiang.length === 0 ? (
            <div className="text-center py-10">
                <p className="text-gray-500 text-lg">Chưa có bài giảng nào trong lớp này.</p>
                <p className="text-sm text-gray-400 mt-1">Hãy bấm "Tạo bài giảng mới" để bắt đầu!</p>
            </div>
         ) : (
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {danhSachBaiGiang.map((bg: any, index: number) => (
                  <tr key={bg.id || index} className="border-b border-gray-200 hover:bg-gray-100 transition duration-150">
                    
                    {/* 🚀 ĐÂY LÀ PHẦN "BAO LÔ": Kiểu gì cũng phải moi được cái tên ra hiển thị */}
                    <td className="py-3 px-2 font-medium text-blue-600">
                        {bg.tieuDe || bg.title || bg.name || "Bài giảng chưa có tiêu đề"}
                    </td>
                    ...
                    <td className="py-3 px-2 space-x-4 text-center">
                       <button onClick={() => handleMoFormSua(bg)} className="text-blue-500 hover:text-blue-700 font-medium hover:underline">Sửa</button>
                       <button onClick={() => handleXoa(bg.id)} className="text-red-500 hover:text-red-700 font-medium hover:underline">Xóa</button>
                    </td>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50 transition-opacity">
           <div className="bg-white p-6 rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto relative animate-fade-in-up">
              ...
              <FormBaiGiang 
                ...
                onSuccess={() => { setIsModalOpen(false); loadDanhSach(); }} 
              />
           </div>
        </div>
      )}
