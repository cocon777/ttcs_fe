        ...
        <Fragment>
          ...
          >
            ...
            >
              ...
              {/* 🚀 NẾU CHƯA CÓ OPTIONS THÌ BÁO ĐANG TẢI */}
              {!options ? (
                <li className="px-4 py-3 text-center text-slate-400 flex items-center justify-center gap-2">
                ...
                /* 🚀 NẾU CÓ RỒI THÌ MỚI CHẠY MAP (THÊM DẤU ?) */
                options?.map((option: any) => {
                  return (
                    <li
                      key={option.value || option.id} // Đề phòng option ko có id
                      onClick={() => handleChangeValue(option)}
                      className={`flex items-center justify-between hover:cursor-pointer hover:bg-darkmode-700 dark:text-slate-300 ${selectedValue?.value === option.value ? "bg-zinc-100 text-blue-900 dark:bg-darkmode-700 dark:text-blue-400" : "hover:bg-gray-100"}`}
                    >
